...
def generate_employee_hours_chart(employee_data):
    ...
    # Use Hebrew role labels in chart (keep data as-is).
    def map_role(role_value):
        if role_value is None:
            return role_value
        role_text = str(role_value).strip()
        role_key = role_text.lower()
        if role_key in {'pilot', 'טייס'}:
            return 'טייס'
        if role_key in {'attendant', 'flight attendant', 'דייל'}:
            return 'דייל'
        return role_text

    names = [
        f"{row['first_name']} {row['last_name']} ({map_role(row.get('role'))})"
