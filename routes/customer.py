...
def book_flight():
    ...
    if request.method == 'POST':
        ...
        is_business_str = request.form.get('is_business')
        is_business = (is_business_str == 'true')
        ...
        price_col = 'business_price' if is_business else 'economy_price'
        
        # Get price and aircraft_id
        flight_info = query_db(f"SELECT {price_col}, aircraft_id FROM Flight WHERE source_airport_id=%s AND dest_airport_id=%s AND departure_time=%s", 
        ...
        """, (order_code, price, email, source_id, dest_id, time_str))
        ...
        try:
            execute_db("""
                ...
                VALUES (%s, %s, %s, %s, %s)
            """, (order_code, aircraft_id, is_business, seat_row, seat_col))
            
            # Instead of redirecting, set success flag and fall through to render template
            ...
            flash("Seat already taken or invalid. Please try again.", "danger")
