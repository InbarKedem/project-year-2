...
class TestDataCompleteness:
    ...
    def test_all_airport_pairs_have_routes(self):
        ...
        # Get all airports (excluding test airports)
        cursor.execute("SELECT airport_id FROM Airport WHERE airport_id NOT IN (9998, 9999) ORDER BY airport_id")
...
class TestSeedDataPreservation:
    ...
    def test_seed_employees_exist(self):
        ...
        # Check pilots (generated by script: 300000001-300000040, but may vary)
        cursor.execute("SELECT id_number FROM Flight_Crew WHERE id_number LIKE '3000000%' AND is_pilot = TRUE")
        pilots = cursor.fetchall()
        assert len(pilots) >= 5, f"Expected at least 5 seed pilots, found {len(pilots)}"
        
        # Check attendants (generated by script: 400000001-400000040, but may vary)
        cursor.execute("SELECT id_number FROM Flight_Crew WHERE id_number LIKE '4000000%' AND is_pilot = FALSE")
        attendants = cursor.fetchall()
        assert len(attendants) >= 5, f"Expected at least 5 seed attendants, found {len(attendants)}"
